...
class Question(models.Model):
    ...
    model_answer = models.TextField(blank=True, help_text="Model answer or grading criteria")
    ...
    is_verified = models.BooleanField(
        default=False, 
        help_text="Verified by admin/teacher",
        db_index=True
    )
    ...
    def clean(self):
        ...
        if self.question_type in [self.QuestionType.MULTIPLE_CHOICE, self.QuestionType.TRUE_FALSE, self.QuestionType.MATCHING]:
            ...
            if self.question_id and self.question_type in [self.QuestionType.MULTIPLE_CHOICE, self.QuestionType.TRUE_FALSE]:
            ...
            if self.question_type == self.QuestionType.TRUE_FALSE and self.question_id:
                if self.choices.count() != 2:
                    raise ValidationError("True/False questions must have exactly two choices.")
...
class QuestionChoice(models.Model):
    ...
    # For matching questions - if this is null, it's not a matching question pair
    matching_text = models.CharField(max_length=255, blank=True, null=True, 
    ...
    def clean(self):
        ...
        # Ensure matching_text is provided for matching questions
        if self.question.question_type == Question.QuestionType.MATCHING and not self.matching_text:
            raise ValidationError("Matching questions require matching text for each choice.")
            
        # Ensure matching_text is empty for non-matching questions
        if self.question.question_type != Question.QuestionType.MATCHING and self.matching_text:
            raise ValidationError("Matching text should only be used for matching questions.")